...
export default function NewProductForm() {
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-blue-900 to-pink-800 dark:from-gray-900 dark:via-blue-900 dark:to-gray-900 py-24 px-4">
      <div className="max-w-5xl mx-auto relative">
        {/* Header with animated text */}
        <div className="mb-12 text-center">
          <h1 className="text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-pink-300 via-blue-300 to-indigo-300 inline-block">
            Create {category} Product
          </h1>
          <div className="mt-3 text-pink-200 dark:text-blue-200 animate-pulse">
              ...
              <div className="space-y-4">
                <div
                  ...
                  className={`p-3 rounded-lg cursor-pointer transition-all ${activeSection === 'info'
                    ? 'bg-pink-500 text-white font-medium translate-x-2'
                    : 'text-gray-300 hover:bg-blue-800 hover:bg-opacity-40'}`}
                >
                  <div className="flex items-center">
                    <span className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-pink-800 mr-3">1</span>
                    Basic Info
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center">
                    <span className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-pink-800 mr-3">2</span>
                    Main Image
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center">
                    <span className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-pink-800 mr-3">3</span>
                    Gallery Images
                    {Object.keys(form.subimages).length > 0 && (
                      <span className="ml-2 bg-pink-600 text-white rounded-full text-xs px-2 py-1">
                ...
                >
                  <div className="flex items-center">
                    <span className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-pink-800 mr-3">4</span>
                    Part Numbers
                  </div>
                </div>
              </div>
                ...
                <div className={`transition-all duration-300 ${activeSection === 'info' ? 'opacity-100' : 'hidden opacity-0'}`}>
                  ...
                  <div className="relative mt-6 group">
                    <input
                      ...
                      required
                      className="peer w-full bg-transparent text-2xl text-white border-b-2 border-blue-300 border-opacity-50 focus:border-pink-300 outline-none px-2 py-3 placeholder-transparent transition-all"
                      placeholder="Enter part name"
                    />
                    <label
                      htmlFor="part_name"
                      className="absolute left-2 -top-5 text-sm text-blue-200 transition-all peer-placeholder-shown:text-xl peer-placeholder-shown:text-white peer-placeholder-shown:top-3 peer-focus:-top-5 peer-focus:text-sm peer-focus:text-pink-300"
                    >
                      Enter part name
                    </label>
                    ...
                    {form.part_name && (
                      <button
                        type="button"
                        onClick={() => setActiveSection('main_image')}
                        className="mt-8 bg-pink-500 hover:bg-pink-600 text-white px-6 py-3 rounded-full shadow-lg hover:shadow-pink-500/30 transition-all transform hover:translate-y-1"
                      >
                        Continue to Images →
                      </button>
                    )}
                ...
                <div className={`transition-all duration-300 ${activeSection === 'main_image' ? 'opacity-100' : 'hidden opacity-0'}`}>
                  ...
                  <div
                    className={`mt-6 border-2 border-dashed rounded-lg p-12 text-center transition-all ${fileHover
                        ? 'border-pink-400 bg-pink-900 bg-opacity-20'
                        : 'border-blue-300 border-opacity-50 hover:border-pink-400'
                      }`}
                    onDragOver={(e) => {
                      e.preventDefault();
                      setFileHover(true);
                    ...
                    <div className="relative">
                      ...
                      {!form.image ? (
                        <div>
                          ...
                          <p className="mt-4 text-xl text-blue-200">Drop image here or click to browse</p>
                          <p className="mt-2 text-sm text-blue-300">PNG, JPG, GIF up to 10MB</p>
                        </div>
                      ) : (
                        <div>
                      ...
                      {uploading && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70 rounded-lg">
                          ...
                          <div className="text-xs text-pink-200 mt-1">{uploadProgress}%</div>
                        </div>
                      )}
                    </div>
                    <button
                      type="button"
                      onClick={() => setActiveSection('subimages')}
                      className="mt-8 z-[1000] bg-pink-500 hover:bg-pink-600 text-white px-6 py-3 rounded-full shadow-lg hover:shadow-pink-500/30 transition-all transform hover:translate-y-1"
                    >
                      Continue to Gallery →
                    </button>
                  </div>
                ...
                <div className={`transition-all duration-300 ${activeSection === 'subimages' ? 'opacity-100' : 'hidden opacity-0'}`}>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                        {Object.entries(form.subimages).map(([key, url], i) => (
                          <div key={key} className="relative group rounded-lg overflow-hidden">
                            <div className="aspect-square bg-blue-900 bg-opacity-20">
                              <img
                                src={url}
                        ...
                        {Object.keys(form.subimages).length < 10 && (
                          <div
                            className="w-48 h-48 aspect-square border-2 border-dashed border-blue-300 border-opacity-50 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-pink-400 hover:bg-pink-900 hover:bg-opacity-20 transition-all"
                          >
                            <input
                              type="file"
                              id="add-more-images"
                            ...
                            <span className="text-xs text-blue-300 mt-2">Add More</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {Object.keys(form.subimages).length === 0 && (
                    ...
                    >
                      <div className="relative">
                        ...
                        <div>
                          <svg className="w-12 h-12 mx-auto text-blue-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                          </svg>
                          <p className="mt-3 text-lg text-blue-200">Drop multiple images or click to browse</p>
                          <p className="mt-2 text-sm text-blue-300">Select up to 10 gallery images</p>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="fixed inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70 z-50">
                      <div className="bg-gray-900 p-8 rounded-xl max-w-md w-full">
                        <div className="text-pink-300 flex flex-col items-center">
                          ...
                          <div className="text-sm text-pink-200">{uploadProgress}% complete</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="mt-8 flex justify-between">
                    <button
                      type="button"
                      onClick={() => setActiveSection('main_image')}
                      className="bg-blue-800 bg-opacity-40 hover:bg-opacity-60 text-white px-6 py-2 rounded-full transition-all"
                    >
                      ← Back
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveSection('part_number')}
                      className="bg-pink-500 hover:bg-pink-600 text-white px-6 py-2 rounded-full shadow-lg hover:shadow-pink-500/30 transition-all"
                    >
                      Continue →
                    </button>
                  </div>
                ...
                <div className={`transition-all duration-300 ${activeSection === 'part_number' ? 'opacity-100' : 'hidden opacity-0'}`}>
                  ...
                  <div className="space-y-6">
                    {Object.keys(form.part_number).map((engine) => (
                      <div key={engine} className="bg-blue-900 bg-opacity-20 backdrop-blur-sm p-6 rounded-lg">
                        <label className="block text-lg text-blue-200 mb-3">
                          {engine}
                        </label>
                        <div className="relative">
                          <input
                            ...
                            onChange={handleChange}
                            className="w-full bg-blue-800 bg-opacity-30 text-white border-2 border-blue-500 border-opacity-50 focus:border-pink-400 rounded-lg px-4 py-3 outline-none transition-all"
                  ...
                  <div className="mt-12">
                    <button
                      type="submit"
                      disabled={uploading}
                      className="relative w-full bg-gradient-to-r from-pink-500 to-blue-600 text-white font-medium py-4 px-8 rounded-xl shadow-xl hover:shadow-pink-500/20 hover:from-pink-600 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden"
                    >
                    ...
                    <div className="flex justify-between mt-6">
                      <button
                        type="button"
                        onClick={() => setActiveSection('subimages')}
                        className="text-blue-300 hover:text-pink-300 text-sm"
                      >
                        ← Back to Gallery
                      </button>
